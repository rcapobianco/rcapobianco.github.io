...
Now we need to distinguish between the possible domains of existence for orbits. First, noting that: $ 0 \leq  1-\cos\theta \leq 2 $, therefore there are three different regimes: when $\mathcal{E} < 2$, and thus there is a "competition" between these two comparable contributions, leading to a bound motion. The case $\mathcal{E} = 2$ signals that the system has almost exact energy needed to reach the maximum value $\theta_{max} = \pi$, this is the system's separatrix. The last case is $\mathcal{E} > 2$, here the square root is positive regardless of values assumed by $\theta$, the system can do complete circular rotations, and therefore the orbit is unbound.
