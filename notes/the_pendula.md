...
It is clear then that we must have: $\theta_0 \leq \theta_{\max}$; therefore, for a given value of $\mathcal{E}$, the motion is only allowed between $( -\theta_{\max}, \theta_{\max} )$. From here, it is easy to verify that for a particle starting from rest 

$$ \dot{\theta}_0 = 0 \rightarrow \theta_{\max} = \theta_0 $$
...
The double pendulum is simply two pendulums coupled to each other, and thus the motion of both masses depends on the drag generated by the other. The equations of motion can be evaluated quite simply; however, integrability is completely lost, and thus the system is chaotic. To obtain the equations of motion, consider the coordinates described in Fig. 2:

<div style="display:flex; align-items:flex-start; gap:20px;">

  <!-- Left column: image -->
  <div style="flex:0 0 35%; max-width:220px;">
    <img src="/assets/images/double_pendulum_coordinates.png" style="width:100%; display:block;">
    <p style="font-size:14px; text-align:center; margin-top:5px;">
...
x_2 &= x_1 + \ell_2\sin\theta_2 , & y_2 &= y_1 - \ell_2\cos\theta_2 
\end{aligned}
$$

from which the velocities can be easily evaluated:

$$
\begin{aligned}
\dot{x}_1 &= \ell_1 \dot{\theta}_1 \cos\theta_1 , & \dot{y}_1 &= \ell_1 \dot{\theta}_1 \sin\theta_1 \\
\dot{x}_2 &= \dot{x}_1 + \ell_2 \dot{\theta}_2 \cos\theta_2 , & \dot{y}_2 &= \dot{y}_1 + \ell_2 \dot{\theta}_2 \sin\theta_2 
\end{aligned}
$$

and then the kinetic energy is obtained after some algebra:

$$
\begin{aligned}
\mathcal{K} &= \frac{m_1}{2} \left(\dot{x}_1^2 + \dot{y}_1^2\right) + \frac{m_2}{2} \left( \dot{x}_2^2 + \dot{y}_2^2 \right) \\
&= \frac{(m_1 + m_2) \ell_1^2}{2} \dot{\theta}_1^2 + \frac{m_2 \ell_2^2}{2} \dot{\theta}_2^2 + m_2 \ell_1 \ell_2 \dot{\theta}_1 \dot{\theta}_2 \cos\Delta 
\end{aligned}
$$
{% endraw %}
...
{% raw %}
$$
U = - (m_1 + m_2) g \ell_1 (1-\cos\theta_1) - m_2 g \ell_2 (1-\cos\theta_2) 
$$
{% endraw %}
...
$$
\begin{aligned}
(m_1+m_2)\ell_1 \ddot{\theta}_1 + m_2 \ell_2 \cos\Delta \, \ddot{\theta}_2 &= - m_2 \ell_2 \sin\Delta \, \dot{\theta}_2^2 - (m_1+m_2) g \sin\theta_1 \\
m_2 \ell_1 \cos\Delta \, \ddot{\theta}_1 + m_2 \ell_2 \ddot{\theta}_2 &= m_2 \ell_1 \sin\Delta \, \dot{\theta}_1^2 - m_2 g \sin\theta_2 
\end{aligned}
$$
{% endraw %}
...
where $ \mathbf{M}(\boldsymbol{\theta}) $ is the <em>mass matrix</em>. Illustratively, one can see this as a <em>Newton's second law</em> form, where the "mass" is represented by a matrix in a coupled system, instead of a constant in a simple case. To solve the system of equations, we first linearize it. Here, linearizing simply means representing the system as one coupled differential equation per acceleration. After diagonalizing the mass matrix, one finds:
